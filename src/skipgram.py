...
def train_skipgram(
        ...
        save_path: str = 'skipgram_model.pt'
) -> Tuple[SkipGram, Word2VecVocab]:
    # Create vocabulary
    ...
    for epoch in epoch_bar:
        ...
        for batch_idx, (center_words, context_words) in batch_bar:
            center_words = center_words.to(device)
            context_words = context_words.to(device)

            # Generate negative samples
            negative_samples = vocab.sample_negative_words(
                center_words.size(0), num_negative
            ).to(device)

            # Forward pass
            pos_score, neg_score = model(center_words, context_words, negative_samples)
    ...
    # Save model
    save_skipgram_model(model, vocab, save_path)

    return model, vocab


def save_skipgram_model(model: SkipGram, vocab: Word2VecVocab, save_path: str):
    """Save Skip-gram model, embeddings and vocabulary to file"""
    save_data = {
    ...
    logging.info(f"Saved Skip-gram model to {save_path}")


def load_skipgram_model(load_path: str, device=None) -> Tuple[SkipGram, Word2VecVocab, Dict]:
    """Load saved Skip-gram model, embeddings and vocabulary"""
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    # Reconstruct model
    model = SkipGram(len(vocab), save_data['metadata']['embedding_dim']).to(device)
    model.load_state_dict(save_data['model_state_dict'])

    return model, vocab, save_data['metadata']


def find_similar_words(word: str, vocab: Word2VecVocab, model: SkipGram, top_k: int = 5) -> List[Tuple[str, float]]:
    ...
    most_similar = []
    for idx in top_indices[1:top_k + 1]:
